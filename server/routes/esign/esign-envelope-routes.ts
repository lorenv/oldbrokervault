...
router.post('/envelopes/:id/send', async (req: Request, res: Response) => {
  try {
    ...
    for (const recipient of recipientsToNotify) {
      ...
      const signingUrl = `${process.env.APP_URL || process.env.BASE_URL || ''}/esign/sign/${recipient.accessToken}`;
...
router.post('/sign/:token/complete', async (req: Request, res: Response) => {
  try {
    ...
    if (allSigned) {
      ...
      const envelopeUrl = `${process.env.APP_URL || process.env.BASE_URL || ''}/esign/envelope/${envelope.envelopeId}`;
      ...
      if (nextSigner) {
        ...
        const signingUrl = `${process.env.APP_URL || process.env.BASE_URL || ''}/esign/sign/${nextSigner.accessToken}`;
