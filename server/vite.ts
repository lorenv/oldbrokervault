...
export async function setupVite(app: Express, server: Server) {
  ...
  // Critical fix: Only handle non-API routes with Vite
  app.use("*", async (req, res, next) => {
    const url = req.originalUrl;
    
    // Skip Vite handling for API routes - let them be handled by Express routes
    if (url.startsWith('/api/')) {
      console.log(`🔍 Skipping Vite for API route: ${url}`);
      return next();
    }
...
export function serveStatic(app: Express) {
  ...
  app.use("*", (req, res) => {
    ...
    if (req.originalUrl.startsWith('/api/')) {
      console.log(`🔍 Skipping static serving for API route: ${req.originalUrl}`);
