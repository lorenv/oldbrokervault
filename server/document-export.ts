...
/**
 * Generates Rich Text Format (RTF) for the CIM data
 * Questions are formatted with larger font size and bold text compared to answers
 * @param analysis The CIM analysis data
 * @returns RTF document as a string
 */
export function generateRichText(analysis: any): string {
  // RTF header and font table
  let rtf = `{\\rtf1\\ansi\\ansicpg1252\\cocoartf2580
\\cocoatextscaling0\\cocoaplatform0{\\fonttbl\\f0\\fswiss\\fcharset0 Helvetica;\\f1\\fswiss\\fcharset0 Helvetica-Bold;}
{\\colortbl;\\red51\\green51\\blue51;\\red31\\green39\\blue51;\\red75\\green85\\blue99;}
\\paperw12240\\paperh15840\\margl1440\\margr1440\\vieww11520\\viewh8400\\viewkind0
\\pard\\tx720\\tx1440\\tx2160\\tx2880\\tx3600\\tx4320\\tx5040\\tx5760\\tx6480\\tx7200\\tx7920\\tx8640\\pardirnatural\\partightenfactor0

\\f1\\fs36 \\cf1 CONFIDENTIAL INFORMATION MEMORANDUM\\par
\\pard\\tx720\\tx1440\\tx2160\\tx2880\\tx3600\\tx4320\\tx5040\\tx5760\\tx6480\\tx7200\\tx7920\\tx8640\\pardirnatural\\partightenfactor0
\\f0\\fs24 \\cf1 \\
`;

  // Business Overview Section
  rtf += `\\f1\\fs32 \\cf2 BUSINESS OVERVIEW\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 When was the business founded?\\par
\\f0\\fs24 \\cf1 ${analysis.story?.yearStarted || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What is the business structure?\\par
\\f0\\fs24 \\cf1 ${analysis.story?.businessStructure || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What is the business model?\\par
\\f0\\fs24 \\cf1 ${analysis.story?.businessModel || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What process do customers follow to place orders?\\par
\\f0\\fs24 \\cf1 ${analysis.story?.orderProcess || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 How has the business grown over time?\\par
\\f0\\fs24 \\cf1 ${analysis.story?.growthHistory || 'Information not provided'}\\par
\\
`;

  // Business Summary
  rtf += `\\f1\\fs28 \\cf3 Business Summary\\par
\\f0\\fs24 \\cf1 ${analysis.story?.businessSummary || 'No detailed business summary provided.'}\\par
\\
`;

  // Reason for Sale (if provided)
  if (analysis.story?.saleReason) {
    rtf += `\\f1\\fs28 \\cf3 Reason For Sale\\par
\\f0\\fs24 \\cf1 ${analysis.story.saleReason}\\par
\\
`;
  }

  // Investment Highlights
  rtf += `\\f1\\fs32 \\cf2 INVESTMENT HIGHLIGHTS\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 What makes this business attractive to buyers?\\par
\\f0\\fs24 \\cf1 \\
`;
  ...
  if (analysis.executiveSummary?.buyerAttractions?.length || analysis.story?.keyAttractions?.length) {
    const attractions = analysis.executiveSummary?.buyerAttractions || analysis.story?.keyAttractions || [];
    attractions.forEach((item: string) => {
      rtf += `\\f0\\fs24 \\cf1 • ${item}\\par
`;
    });
  } else {
    rtf += `\\f0\\fs24 \\cf1 Information not provided.\\par
`;
  }
  
  rtf += `\\
\\f1\\fs28 \\cf3 What growth opportunities exist for new ownership?\\par
\\f0\\fs24 \\cf1 \\
`;
...
`;
  ...
  // Market Analysis
  rtf += `\\f1\\fs32 \\cf2 MARKET ANALYSIS\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 Who is the target customer?\\par
\\f0\\fs24 \\cf1 ${analysis.marketAnalysis?.customerProfile || 'No customer profile information provided.'}\\par
\\
`;

  rtf += `\\f1\\fs28 \\cf3 What makes this business unique in the market?\\par
\\f0\\fs24 \\cf1 \\
`;
...
`;
  ...
  rtf += `\\f1\\fs28 \\cf3 Who are the key competitors?\\par
\\f0\\fs24 \\cf1 \\
`;
...
`;
  ...
  rtf += `\\f1\\fs28 \\cf3 What are the business strengths?\\par
\\f0\\fs24 \\cf1 \\
`;
...
`;
  ...
  // Operations Section
  rtf += `\\f1\\fs32 \\cf2 BUSINESS OPERATIONS\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 Customer Relationships\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 How much revenue is recurring?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.customers?.recurring || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 How would you describe your customer relationships?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.customers?.relationships || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 Is revenue concentrated among certain customers?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.customers?.concentration || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What are the typical contract terms?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.customers?.contracts || 'Information not provided'}\\par
\\
`;

  rtf += `\\f1\\fs28 \\cf3 Supply Chain\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 How many suppliers does the business work with?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.suppliers?.count || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What are the supplier payment terms?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.suppliers?.terms || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 Is there concentration among certain suppliers?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.suppliers?.concentration || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 How easily can supplier relationships be transferred?\\par
\\f0\\fs24 \\cf1 ${analysis.operations?.suppliers?.transferability || 'Information not provided'}\\par
\\
`;

  // Team & Organization
  rtf += `\\f1\\fs32 \\cf2 TEAM & ORGANIZATION\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 What are the owner's current responsibilities?\\par
\\f0\\fs24 \\cf1 ${analysis.team?.ownerResponsibilities || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 How many hours per week does the owner work?\\par
\\f0\\fs24 \\cf1 ${analysis.team?.ownerHours || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 How many employees does the business have?\\par
\\f0\\fs24 \\cf1 ${analysis.team?.employeeCount || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What is the management structure?\\par
\\f0\\fs24 \\cf1 ${analysis.team?.management || 'Information not provided'}\\par
\\
`;

  // Facilities
  rtf += `\\f1\\fs32 \\cf2 FACILITIES\\par
\\f0\\fs24 \\cf1 \\
`;

  rtf += `\\f1\\fs28 \\cf3 What is the ownership status of the facility?\\par
\\f0\\fs24 \\cf1 ${analysis.facility?.ownership || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What is the size of the facility?\\par
\\f0\\fs24 \\cf1 ${analysis.facility?.size || 'Information not provided'}\\par
\\
\\f1\\fs28 \\cf3 What is the monthly cost of the facility?\\par
\\f0\\fs24 \\cf1 ${analysis.facility?.cost || 'Information not provided'}\\par
\\
`;

  if (analysis.facility?.leaseDetails) {
    rtf += `\\f1\\fs28 \\cf3 What are the lease details?\\par
\\f0\\fs24 \\cf1 ${analysis.facility.leaseDetails}\\par
\\
`;
  }
