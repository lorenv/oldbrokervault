...
// Draw branding elements directly onto a page (no embedded template - draws elements directly)
async function drawBrandingOnPage(
  page: pdfLib.PDFPage,
  doc: pdfLib.PDFDocument,
  brandedTemplate: string,
  brandColors: string[] | null,
  logoImage: pdfLib.PDFImage | null
): Promise<void> {
  const primaryColor = brandColors && brandColors[0] ? hexToRgb(brandColors[0]) : { r: 0.2, g: 0.4, b: 0.6 };
  const secondaryColor = brandColors && brandColors[1] ? hexToRgb(brandColors[1]) : { r: 0.9, g: 0.9, b: 0.9 };
  ...
  switch (brandedTemplate) {
    case 'watermark':
      // Diagonal watermark text - NO BACKGROUND
      if (logoImage) {
        const logoScale = Math.min(200 / logoImage.width, 200 / logoImage.height, 1);
        const logoWidth = logoImage.width * logoScale;
        const logoHeight = logoImage.height * logoScale;
        page.drawImage(logoImage, {
    ...
    default:
      console.log(`Unknown branded template in drawBrandingOnPage: ${brandedTemplate}`);
  }
}

// Apply branded overlay ON TOP OF existing pages (draws branding elements AFTER content)
async function applyBackgroundToPages(
  originalPdfBuffer: Buffer,
  backgroundTemplate: pdfLib.PDFDocument,
  brandedTemplate?: string,
  brandColors?: string[] | null,
  businessLogo?: string | null
): Promise<Buffer> {
  try {
    // Load the original PDF generated by PDFKit - we'll modify it directly
    ...
    console.log("Original PDF has", originalPages.length, "pages - applying branded overlay to ALL pages");
    console.log("Branding template:", brandedTemplate);
    console.log("Brand colors:", brandColors);
    console.log("Has business logo:", !!businessLogo);

    // Load logo if available - must embed into the target document
    let logoImage: pdfLib.PDFImage | null = null;
    if (businessLogo) {
      try {
        let logoBuffer: Buffer | null = null;
        ...
        if (logoBuffer) {
          try {
            const pngBuffer = await sharp(logoBuffer).png().toBuffer();
            logoImage = await originalDoc.embedPng(pngBuffer);
            console.log("Logo embedded successfully");
          } catch (pngError) {
            try {
              const jpgBuffer = await sharp(logoBuffer).jpeg().toBuffer();
              logoImage = await originalDoc.embedJpg(jpgBuffer);
    ...
    // Draw branding directly onto each page
    console.log(`Drawing branding overlay on ${originalPages.length} pages...`);
    const overlayStart = Date.now();

    // For watermark template, skip first and last pages (they have other content that would conflict)
    const isWatermark = brandedTemplate === 'watermark';
    ...
    for (let i = 0; i < totalPages; i++) {
      try {
        ...
        // Skip first and last pages for watermark template only
        if (isWatermark && (isFirstPage || isLastPage)) {
          console.log(`Skipping watermark branding on page ${i + 1} (${isFirstPage ? 'first' : 'last'} page)`);
          continue;
        }

        // Draw branding elements directly ON TOP of existing content
        if (brandedTemplate && brandedTemplate !== 'none') {
          await drawBrandingOnPage(page, originalDoc, brandedTemplate, brandColors || null, logoImage);
        }

        if (i % 10 === 0) {
          console.log(`Applied branding to page ${i + 1} of ${totalPages}...`);
        }

      } catch (pageError) {
        console.error(`Error applying branding to page ${i + 1}:`, pageError);
        // Continue with other pages
      }
    }

    console.log(`Branding overlay applied in ${Date.now() - overlayStart}ms`);

    // Save the modified PDF with branding applied
    const finalPdfBytes = await originalDoc.save();
    console.log(`Branded overlay applied to ${originalPages.length} pages`);
...
export async function generatePDF(analysis: any, logoUrl?: string | null, websiteUrl?: string, selectedImages?: string[], userProfile?: any, financialData?: any, financialFiles?: any[], baseUrl?: string, documentTitle?: string, customSections?: any[], coverImageUrl?: string | null, coverImagePosition?: string | null, documentId?: number, pdfTemplate?: string, shareSlug?: string, brandedPdfTemplate?: string, brandColors?: string[] | null, businessLogo?: string | null): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    ...
    console.log("🎨 Brand colors:", JSON.stringify(brandColors));
    ...
    // Debug the condition
    console.log("🎨 Condition check - brandedPdfTemplate:", brandedPdfTemplate);
    console.log("🎨 Condition check - brandedPdfTemplate !== 'none':", brandedPdfTemplate !== 'none');
    console.log("🎨 Condition check - brandColors:", !!brandColors);
    console.log("🎨 Condition check - businessLogo:", !!businessLogo);
    console.log("🎨 Condition check - (brandColors || businessLogo):", !!(brandColors || businessLogo));
    ...
    if (brandedPdfTemplate && brandedPdfTemplate !== 'none' && (brandColors || businessLogo)) {
      ...
      backgroundTemplatePromise = generateBrandedPdfTemplate(brandedPdfTemplate, brandColors || null, businessLogo || null, baseUrl).then(result => {
        console.log("✅ generateBrandedPdfTemplate returned:", result ? "PDFDocument" : "null");
        return result;
      }).catch(err => {
        console.error("❌ generateBrandedPdfTemplate error:", err);
        return null;
      });
    ...
    doc.on('end', async () => {
      try {
        ...
        fs.appendFileSync('/tmp/pdf-debug.log', `backgroundTemplate exists: ${!!backgroundTemplate}\n`);
        if (backgroundTemplate) {
          fs.appendFileSync('/tmp/pdf-debug.log', `Applying background template...\n`);
          fs.appendFileSync('/tmp/pdf-debug.log', `brandedPdfTemplate: ${brandedPdfTemplate}\n`);
          fs.appendFileSync('/tmp/pdf-debug.log', `brandColors: ${JSON.stringify(brandColors)}\n`);
          fs.appendFileSync('/tmp/pdf-debug.log', `businessLogo: ${businessLogo}\n`);
          console.log("Applying optimized PDF background template...");
          const startTime = Date.now();
          // Pass branding parameters to draw branding ON TOP of content (not as background)
          let finalPdfBuffer = await applyBackgroundToPages(
            originalPdfBuffer,
            backgroundTemplate,
            brandedPdfTemplate,
            brandColors,
          ...
          fs.appendFileSync('/tmp/pdf-debug.log', `Background applied in ${processingTime}ms, size: ${finalPdfBuffer.length}\n`);
          ...
          fs.appendFileSync('/tmp/pdf-debug.log', `No background template - returning original PDF\n`);
