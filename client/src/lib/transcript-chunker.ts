...
export function mergeAnalysisResults(results: any[]): any {
  ...
  if (merged.BusinessDescription) {
    // Take the longest business description
    const descriptions = results.map(r => 
      r.BusinessDescription?.Summary?.Text || ''
    );
    merged.BusinessDescription.Summary.Text = descriptions.reduce((a, b) => 
      a.length > b.length ? a : b
    );

    // Merge arrays in BusinessDescription
    if (Array.isArray(merged.BusinessDescription.KeyDifferentiators)) {
      merged.BusinessDescription.KeyDifferentiators = processArrayField(
        merged.BusinessDescription, 'KeyDifferentiators'
      );
    }
  }
