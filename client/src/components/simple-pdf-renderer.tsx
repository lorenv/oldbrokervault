...
export default function SimplePdfRenderer({
  pdfBase64,
  signatureFields,
  onFieldsChange,
  selectedFieldType
  ...
  useEffect(() => {
    ...
    const renderPdf = async () => {
      ...
      try {
        console.log('Simple PDF renderer starting...');
        ...
        const scale = Math.min((containerWidth - 40) / viewport.width, 1.2);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
        <div className="flex items-center gap-2">
          <span className="text-sm">PDF Template Editor - Page 1</span>
      ...
      {/* Instructions */}
      <Card className="p-4">
        <h4 className="font-medium mb-2">Simple PDF Template Editor</h4>
        <ul className="text-sm text-gray-600 space-y-1">
          <li>• PDF displayed above - click anywhere to add signature fields</li>
          <li>• Drag fields to reposition them precisely</li>
          <li>• Double-click field labels to edit them</li>
          <li>• Field coordinates are saved for signature placement</li>
        </ul>
      </Card>
    </div>
  );
